...
const navSections: NavSection[] = [
  {
    items: [
      ...
      { href: '/app/needs-attention', label: 'Needs Attention', icon: AlertTriangle },
    ],
  },
  {
    title: 'Inventory Management',
    items: [
      { href: '/app/my-items', label: 'My Items', icon: FolderOpen },
      { href: '/app/inventory', label: 'Inventory', icon: Package },
      { href: '/app/receiving', label: 'Receiving', icon: PackageCheck, minRole: 'STAFF' },
      { href: '/app/stock-count', label: 'Stock Count', icon: ClipboardCheck, minRole: 'STAFF' },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/app/reorder-suggestions', label: 'Reorder Suggestions', icon: ListPlus, minRole: 'STAFF' },
    ],
  },
  {
    title: 'Suppliers',
    items: [
      ...
      { href: '/app/supplier-catalog', label: 'Supplier Catalog', icon: BookOpen },
    ],
  },
  {
    title: 'Settings',
    items: [
      { href: '/app/settings', label: 'Settings', icon: Settings, minRole: 'STAFF' },
    ],
  },
];
...
export function ClinicSidebar({ practiceName, userRole, isOpen, onClose }: ClinicSidebarProps) {
  ...
  useEffect(() => {
    setMounted(true);
    const stored = localStorage.getItem('clinic-sidebar-collapsed');
    if (stored !== null) {
  ...
  const toggleCollapse = () => {
    ...
    localStorage.setItem('clinic-sidebar-collapsed', String(newState));
  };
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 z-50 flex h-screen flex-col border-r border-slate-200/60 bg-white/90 backdrop-blur-xl shadow-sm transition-transform duration-300 dark:border-slate-700/50 dark:bg-slate-900/90 dark:shadow-md md:static md:z-auto ${
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between border-b border-slate-200/60 px-4 dark:border-slate-700/50">
          {(!isCollapsed || !mounted) && (
