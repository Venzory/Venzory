...
export function LowQualityTable({ products }: LowQualityTableProps) {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (product: LowQualityProduct) => (
        <Link
          href={`/owner/product-master/${product.id}`}
          className="inline-flex items-center gap-1 text-sm font-medium text-sky-600 transition hover:text-sky-700 dark:text-sky-400 dark:hover:text-sky-300"
        >
          View
