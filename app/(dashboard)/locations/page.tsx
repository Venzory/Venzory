  ...
  return (
    <div className="space-y-4">
      {locations.map((location: any) => {
        ...
        return (
          <div key={location.id} className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
              ...
              <div className="space-y-1">
                ...
                {location.parent ? (
                  <p className="text-xs text-slate-400">
                    Parent: <span className="text-slate-200">{location.parent.name}</span>
                  </p>
                ) : (
                  <p className="text-xs text-slate-500">Top level location</p>
                )}
                <p className="text-xs uppercase tracking-wide text-slate-500">
                  Total quantity recorded: <span className="text-slate-200">{totalQuantity}</span>
                </p>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Items on hand</h3>
              ...
              {canManage ? (
                <details className="rounded-lg border border-slate-800 bg-slate-950/40 p-3 text-sm text-slate-200">
                  <summary className="cursor-pointer text-sm font-medium text-slate-200">Edit location</summary>
                  <form action={upsertLocationInlineAction} className="mt-3 space-y-3">
                    <input type="hidden" name="locationId" value={location.id} />
                    <div className="space-y-1">
                      <label className="text-xs text-slate-400" htmlFor={`location-name-${location.id}`}>
                        Name
                      </label>
                      <input
                    ...
                    <div className="flex gap-3">
                      <div className="flex-1 space-y-1">
                        <label className="text-xs text-slate-400" htmlFor={`location-code-${location.id}`}>
                          Code
                        </label>
                        <input
                      ...
                      <div className="flex-1 space-y-1">
                        <label className="text-xs text-slate-400" htmlFor={`location-parent-${location.id}`}>
                          Parent
                        </label>
                        <select
                    ...
                    <div className="space-y-1">
                      <label className="text-xs text-slate-400" htmlFor={`location-description-${location.id}`}>
                        Description
                      </label>
                      <textarea
