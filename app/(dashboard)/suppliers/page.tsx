  ...
  return (
    <div className="space-y-4">
      {suppliers.map((supplier: any) => (
        <div key={supplier.id} id={supplier.id} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 dark:shadow-none">
            ...
            <div className="space-y-1">
              ...
              <div className="text-sm text-slate-700 dark:text-slate-300">
                {supplier.email ? (
                  <p>
                    Email:{' '}
                    <a href={`mailto:${supplier.email}`} className="text-sky-600 hover:text-sky-700 dark:text-sky-400 dark:hover:text-sky-300">
                      {supplier.email}
                    </a>
                  </p>
                ) : null}
                {supplier.phone ? (
                  <p>
                    Phone:{' '}
                    <a href={`tel:${supplier.phone}`} className="text-sky-600 hover:text-sky-700 dark:text-sky-400 dark:hover:text-sky-300">
                      {supplier.phone}
                    </a>
                  </p>
                ) : null}
                {supplier.website ? (
                  <p>
                    Website:{' '}
                    <a href={supplier.website} className="text-sky-600 hover:text-sky-700 dark:text-sky-400 dark:hover:text-sky-300" target="_blank" rel="noreferrer">
                      {supplier.website}
                    </a>
                  </p>
                ) : null}
              </div>
              {supplier.notes ? <p className="text-sm text-slate-600 dark:text-slate-400">{supplier.notes}</p> : null}
            </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-400">Default items</h3>
              {supplier.defaultItems.length ? (
                <ul className="space-y-1 text-sm text-slate-700 dark:text-slate-300">
                  {supplier.defaultItems.map((item: any) => (
                    <li key={item.id} className="flex items-center justify-between">
                      <span>{item.name}</span>
                      {item.sku ? <span className="text-xs text-slate-500 dark:text-slate-500">{item.sku}</span> : null}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-xs text-slate-500 dark:text-slate-500">No catalog items linked yet.</p>
              )}
            </div>

            {canManage ? (
              <details className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-800 dark:bg-slate-950/40">
                <summary className="cursor-pointer text-sm font-medium text-slate-900 dark:text-slate-200">Edit supplier</summary>
                <form action={upsertSupplierInlineAction} className="mt-3 space-y-3">
                  <input type="hidden" name="supplierId" value={supplier.id} />
                  <div className="space-y-1">
                    <label className="text-xs text-slate-600 dark:text-slate-400" htmlFor={`supplier-name-${supplier.id}`}>
                      Name
                    </label>
                    <input
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-1">
                      <label className="text-xs text-slate-600 dark:text-slate-400" htmlFor={`supplier-email-${supplier.id}`}>
                        Email
                      </label>
                      <input
                    ...
                    <div className="space-y-1">
                      <label className="text-xs text-slate-600 dark:text-slate-400" htmlFor={`supplier-phone-${supplier.id}`}>
                        Phone
                      </label>
                      <input
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-600 dark:text-slate-400" htmlFor={`supplier-website-${supplier.id}`}>
                      Website
                    </label>
                    <input
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-600 dark:text-slate-400" htmlFor={`supplier-notes-${supplier.id}`}>
                      Notes
                    </label>
                    <textarea
